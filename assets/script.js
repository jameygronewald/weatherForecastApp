$(document).ready(function () {
  ...
  const getWeatherData = city => {
    let queryURL =
      ...
      city +
      '&APPID=207015d3d9ea763c8fa74acf5fe16ce5';
    ...
    $.ajax({
      ...
      let queryURL_UV =
        'https://api.openweathermap.org/data/2.5/uvi?APPID=207015d3d9ea763c8fa74acf5fe16ce5&lat=' +
        latitude +
        '&lon=' +
        longitude;
      $.ajax({
      ...
      let queryURLDaily =
        ...
        '&lon=' +
        longitude +
        '&exclude=current,minutely,hourly&appid=207015d3d9ea763c8fa74acf5fe16ce5';
      $.ajax({
        url: queryURLDaily,
        method: 'GET',
      }).then(function (response) {
        ...
        while (i < 6) {
          ...
          let dailyTempFarenheit = (
            (response.daily[i].temp.day - 273.15) * 1.8 +
            32
          ).toFixed(1);
          $('.cardRow').append(
            $(
              '<div class="card col-md-2.4 bg-primary text-white"><div class="card-body"><h5 class="card-title" id = "day' +
                i +
                '"></h5><img src = "https://openweathermap.org/img/wn/' +
                response.daily[i].weather[0].icon +
                '@2x.png" alt = "' +
                response.daily[i].weather[0].description +
                '"><p class="card-text cardTemp"id = "day' +
                i +
                'Temp"></p><p class="card-text cardHumidity" id = "day' +
                i +
                'Humidity"></p></div></div>'
            )
          );
          ...
          $('#day' + i + 'Temp').text(
            'Temperature: ' + dailyTempFarenheit + '°F'
          );
          $('#day' + i + 'Humidity').text(
  ...
  $('.cityButtons').on('click', function (event) {
    event.preventDefault();
    searchInput = $(event.target).attr('data-city');
    getWeatherData(searchInput);
  });
});
